...
func findAllAntinodes(positions map[string][]position, matrix [][]string) int {
	for _, value := range positions {
		...
		for _, value := range combinations {
			...
			for newPositionRow >= 0 && newPositionRow < len(matrix) && newPositionCol >= 0 && newPositionCol < len(matrix[0]) {
				if matrix[newPositionRow][newPositionCol] == "." {
					matrix[newPositionRow][newPositionCol] = "#"
				}
				newPositionRow += deltaY
				newPositionCol += deltaX
			}
			...
			for newPositionRow >= 0 && newPositionRow < len(matrix) && newPositionCol >= 0 && newPositionCol < len(matrix[0]) {
				...
				newPositionRow += deltaY * (-1)
				newPositionCol += deltaX * (-1)
			}
		}
	}
